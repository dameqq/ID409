...
folder_path = "C:\\Users\\Damian\\OneDrive\\Obrazy\\Dokumenty\\PomiarymocysygnałuProjektBadawczy\\poziom3"  # <- Zmień na odpowiedn
...
for filename in os.listdir(folder_path):
    ...
    m = re.match(r"4_(\d+)([dh])?\.csv", filename)
    if m:
        rp_number = int(m.group(1))
        suffix = m.group(2)  # 'd', 'h' lub None

        old_path = os.path.join(folder_path, filename)
        new_filename = f"3_{rp_number}"
        if suffix:
            new_filename += suffix
        new_filename += ".csv"
